...
export default function Page() {
  ...
  const handleAccordionClick = async (id: string, page: number = 1) => {
    if (openAccordion === id && currentPage[id] === page) {
    ...
    try {
      const limit = 4;
      const data = await getOfferByOutletId(id, limit, page);
              ...
              <AccordionContent>
                <div className="md:flex block">
                  ...
                  <div className="lg:flex hidden flex-col pl-14 ">
                    ...
                    {offerByOutletId[outlet.id]?.length > 0 && (
                      <div className="flex justify-center mt-4 space-x-2">
                        <Button
                          disabled={currentPage[outlet.id] === 1}
                          onClick={() =>
                            handleAccordionClick(
                              outlet.id,
                              currentPage[outlet.id] - 1
                            )
                          }>
                          Prev
                        </Button>
                        <span className="px-2">
                          Page {currentPage[outlet.id] || 1} of{" "}
                          {totalPages[outlet.id] || 1}
                        </span>
                        <Button
                          disabled={
                            currentPage[outlet.id] === totalPages[outlet.id]
                          }
                          onClick={() =>
                            handleAccordionClick(
                              outlet.id,
